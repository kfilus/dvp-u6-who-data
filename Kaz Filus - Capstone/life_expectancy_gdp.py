...
sns.set_palette("dark")
plt.xticks(rotation=270)
plt.title("Life Expectancy at Birth by Nation")
plt.savefig("avgbar_LEABY.png")
plt.show()
...
#Despite having very different GDPs, citizens of most of the countries have very similar life expectancies at birth. 
...
plt.title("Distribution of Life Expectancy by Nation")
plt.savefig("violin_LEABY.png")
plt.show()
...
sns.set_palette("BuGn",16)
plt.title("GDP between 2000 and 2015 in 6 Nations")
plt.savefig("bar_GDP.png")
plt.show()
...
sns.set_palette("BuPu",16)
plt.title("Life Expectancy between 2000 and 2015 in 6 Nations")
plt.savefig("bar_LEABY.png")
plt.show()
...
#GDP and life expectancy do not seem to be correlated. Most of the countries experienced the same rate of increase in the life expectancies even though they all experienced very different rates of growth in the GDPs. Meanwhile, Zimbabwe, which experienced almost no growth in GDP, had the most drastic increase in life expectancy duriong the course of these years.
#With respect to GDP, China and the USA change the most while Zimbabwe changes the most with repect to life expectancy.
#With so many bars, it's difficult to determine which years had the most change, but it seems Zimbabwe had the greatest increase in life expectancy between 2010 and 2011.
#Zimbabwe had the least change in GDP over time.
#With respect to the life expectency, all but Zimbabwe seem to be similar. All the nations have very different GDPs.
#Seeing both charts, I'd say there doesn't seem to be a correlation between Life Expectancy and GDP.
#The GDP bar graph suggests China and the USA had drastic increases in their GDPs. The dip in Zimbabwe's Life expectancy could be due to war or epidemics. The up and down movement of the GDP in Germany and Mexico is likely due to an unstable economy.
...
plt.savefig("facet_by_year.png")
plt.show()

g1 = sns.FacetGrid(df, col="Country", hue="Year", col_wrap=3, size=4)
g1 = (g1.map(plt.scatter, "GDP", "LEABY", edgecolor="w").add_legend())
sns.set_palette("bright")
plt.savefig("facet_by_country.png")
plt.show()
...
#China Moves most aross the x axis (greatest increase in GDP) while Zimbabwe moves most across the y axis (greatest increase in life expectancy). Since nothing else we've seen has suggested a correlation between GDP and life expectancy, this is not especially surprising. Because there are so many scatterplots to look at, it's a little difficult to compare the trends and get information from them.
...
g3 = (g3.map(plt.plot, "Year", "LEABY",color="purple").add_legend())
...
g2 = (g2.map(plt.plot, "Year", "GDP",color="green").add_legend())
...
#China's economy has been shifting focus from industry to sevices and it's reliance in exports has decreased.
...
#Histories and cultural values are likely more useful in understanding GDP and life expectancy in a given nation than these two variables are in predicting what the other might be.
#More data from more nations over more years would have made the data more reliable. The number of nations examined, more than anything else, was very limiting.
#My facet grid with a chart for each nation was the most useful in comparing the changes in GDP and life expectancy in each nation over time. The two facet grids showing the GDP and life expectancy of the six nations were very helpful in visualizing how those two variables changed in the 6 nations over time.
